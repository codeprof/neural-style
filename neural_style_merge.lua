...
cmd:option('-content_image', 'examples/inputs/brad_pitt.jpg',
...
cmd:option('-style_weight', 0.0) --1e2)
...
cmd:option('-content_layers', 'relu1_1,relu2_1,relu3_1,relu4_1,relu5_1', 'layers for content')
...
function ContentLoss:updateOutput(input)
  ...
  elseif self.mode == 'capture' then
	--torch.save('content' .. content_idx, input:double(),'ascii')
    input = torch.load('merge' .. content_idx, 'ascii'):cuda()
...
function StyleLoss:updateOutput(input)
  ...
  if self.mode == 'capture' then
  	...
  	--torch.save('style' .. style_idx, self.G:double(),'ascii')
